...
7. Input de información de usuario (incluido en las páginas sigIn.html, register.html y las páginas de recoverPassword.html)
...
- **Diseño flexible del layout:**
  Se utiliza `display: flex` junto con `flex-direction: column` en él `body` y otros contenedores clave para garantizar que el contenido se distribuya de manera vertical y ocupe al menos el 100% de la altura de la pantalla (con `min-height: 100vh`).

- **Crecimiento del contenido principal:**
  La propiedad `flex: 1` en él `main` permite que este elemento crezca para empujar él `footer` hacia la parte inferior, asegurando una distribución adecuada del espacio en pantallas de diferentes tamaños.
...
- **Reemplazo de valores en el template:**
  ...
  - Por ejemplo, se reemplaza el texto del `label`, él `placeholder`, el tipo de input y otros atributos para personalizar el elemento de acuerdo a la configuración definida en el JSON.
...
En este sprint se ha desarrollado una transición del proyecto al angular para el frontend así como la implementación de firebase para el backend, es por ello que se hablarán por separados de estos apartados

## Angular 

---

La implementación de angular ha venido del añadido de crear dos tipos de componentes, componentes sobre los templates creados anteriormente asi como componentes que utilizan estos mismos 
templates, ósea todos los contenidos html y css de las páginas se encapsulan en componentes que luego se añaden a páginas cuyo su único propósito es mantener los componentes

Dentro de los componentes se encuentran como dicho antes todos los templates además de: 

1. **about-app:**
    Este se encarga de lo que antes era `index`, teniendo el logo y la descripción del proyecto
2. **add-page-component:**
    Aquí se encuentra la página principal que antes se encontraba en `start`, donde se desarrollaran las conversaciones tanto con los grupos como con los contactos
3. **add-group-component y add-contact:**
    Versiones algo modificadas de `add-group` la cual solo está algo modificada de estructura y `add-contact` la cual está rediseñado para solo necesitar del correo del contacto a añadir.
4. **login-form y register-form:**
    Páginas para el registro y el login que se mantiene igual pero siguiendo la metodología angular
5. **reset-password con todos sus estados:**
    Todos los apartados de cambiar la contraseña que tampoco ha cambiado a un nivel arquitectónico

## Firebase

Por la parte de BackEnd se ha implementado Firebase, una base de datos estándar, hay dos pasos a tener en cuenta, la configuración minima de firebase y la implementación de ello

---

## Configuración mínima de Firebase

Para integrar Firebase en la aplicación, se siguieron los pasos mínimos de inicialización:
...
Se creó el fichero `src/app/environments/environment.ts` donde se añadió la llave entregada por Firebase, aquí hay un ejemplo de como se ve dicho documento:
...
En el arranque de la aplicación `app.config.ts`, se añadió lo siguiente para poder inicializar Firebase:
...
Con esto, Firebase queda conectado y listo para ser utilizado dentro del proyecto con por ejemplo, Firestore o Authentication, que son las utilizadas en el proyecto.

## Implementación de Firebase

---

La implementación de Firebase es bastante más compleja que su configuración así que se dividirá en 2 apartados 

1. La creación de los servicios encerrados en `auth.services`
2. La utilización de esos servicios en los componentes así como su interacción con la base de datos

### Creación de los servicios

Primero se hablará de las funciones creadas en `auth.services` que contiene la capacidad para interactuar con la base de datos mediantes distintos métodos que se agrupan en distintos grupos de contenido

1. **Métodos de Autenticación**
   - `login(email, password)`: permite iniciar sesión con correo y contraseña usando Firebase Auth.
   - `logout()`: cierra la sesión del usuario y limpia él `sessionStorage`.
   - Configuración para que la sesión del usuario se mantenga mientras el navegador esté abierto (`browserSessionPersistence`)

2. **Métodos de creación**
   - `register(...)`: crea una nueva cuenta de usuario en Firebase Auth y guarda su información personal (nombre, apellido, teléfono, etc.) en la colección `usuarios` de Firestore. También se le asigna un avatar inicial y se cierra la sesión tras el registro.
   - `createGroup(...)`: permite crear un grupo nuevo con un avatar predefinido. Busca los UID de los contactos por email, guarda el grupo en Firestore, y actualiza la lista de grupos de cada usuario.
   
3. **Lectura de datos**
   - `getDatosUsuarioPorUID(uid)`: devuelve un observable con los datos de un usuario desde Firestore.
   - `getImg(nombre)`: permite recuperar la url de imágenes (como avatares) desde la colección `img`.
   - `getDatosGrupo(gid)`: devuelve la información de un grupo concreto por su ID.

4. **Edición de contactos y grupos**
   - `addContacto(uid, email)`: añade un nuevo contacto al array de contactos de un usuario.
   - `deleteContacto(uid, contacto)`: elimina un contacto específico del usuario.
   - `addGroup(nombre, descripción, contactos)`: también permite añadir un grupo manualmente (más simple que `createGroup`).


### Utilización de los servicios en los componentes

Una vez implementados los métodos en `auth.service.ts`, se utilizan desde distintos componentes para poder interactuar con la base de datos en Firestore y el sistema de autenticación de Firebase. 

La idea es simple, los servicios se encargan de hacer la conexión con la base de datos para las cosas repetitivas o importantes, mientras los componentes los utilizan como pasos a desarrollar, además sabiendo que todos estos pasos suelen ir de manera muy relacionada siguiendo la metodología CRUD (crear, leer, editar, eliminar)

Esta integración se realiza principalmente a través de **formularios reactivos** y llamadas al servicio mediante `subscribe` o `await`.

Aquí un par de ejemplificaciones de su uso en tres componentes, `add-contact`, `add-group` y `login`

### AddContactComponent

En este componente, el usuario puede buscar otro usuario por su correo electrónico y, si no lo tiene agregado ya, añadirlo como contacto:

- **Paso 1:** Se busca en la colección `usuarios` un dato que coincida con el email introducido.
- **Paso 2:** Si el contacto existe y no está ya en la lista de contactos agregados, se muestra su nombre y avatar.
- **Paso 3:** Al hacer clic en "Añadir contacto", se llama al método `addContacto(...)` del servicio, que actualiza el array `contactos` del usuario actual usando `arrayUnion`.

### AddGroupComponent`

Este componente permite crear un grupo ingresando:

- Nombre del grupo
- Descripción
- Lista de correos de los contactos separados por comas

El proceso es:

- **Paso 1:** Se validan los campos y se limpian los correos.
- **Paso 2:** Se llama a `createGroup(...)` del servicio.
  - Este método busca los UID de los correos proporcionados en Firestore.
  - Luego crea un documento en la colección `grupos` con su avatar, nombre, descripción y lista de miembros.
  - Además, **actualiza a cada usuario** en su colección `usuarios`, añadiéndole el nuevo `gid` en su array de `grupos`.

### LoginFormComponent`

El componente `LoginFormComponent` permite al usuario iniciar sesión mediante su email y contraseña utilizando Firebase Authentication.

El proceso es:

- **Paso 1:** El usuario rellena el formulario reactivo con los campos `email` y `password`, ambos validados localmente (formato, requeridos, etc.).

- **Paso 2:** Al enviar el formulario (`onSubmit()`), se llama al método `login(...)`, que internamente usa `signInWithEmailAndPassword(...)` de Firebase.

- **Paso 3:** Si el login es exitoso, el usuario es redirigido a la página principal (`/chat`); en caso de error (credenciales incorrectas, cuenta inexistente...), se activa una bandera `error` para mostrar un mensaje en pantalla.

Este flujo aprovecha el sistema de suscripción (`.subscribe(...)`) para manejar tanto el éxito como el fallo, dejando al componente encargarse únicamente de la interfaz y la navegación, mientras que la autenticación está totalmente delegada al servicio.

---

Todo esto crea una relación de simbiosis perfecta, angular actúa de frontend creando entornos de compleja compression respecto a html, css y javascript, 
pero que ofrece una gran potencia de uso, pero a necesidad de un backend donde firebase toma el relevo de manera muy sencilla y cuya única complicación es determinada
por la creación y utilización de los servicios
